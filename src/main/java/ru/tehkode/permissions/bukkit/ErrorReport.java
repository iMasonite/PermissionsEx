...
/** Error report builder for PEX that provides additional information on top of report and generates
 * a short URL to create a github issue */
public class ErrorReport {
	...
	public static String shortenURL(String url) {
		...
		try {
			...
			/*
			 * ex.printStackTrace(); // Debug code, uncomment if needed try { if (conn != null) { return
			 * StringUtils.readStream(((HttpURLConnection) conn).getErrorStream()); } } catch (IOException
			 * ignore) { }
			 */
			return url;
		}
	...
	public static String gistText(String text) {
		...
		try {
			...
			if (data.containsKey("html_url")) return data.get("html_url").toString();
		}
		catch (IOException e) {
			e.printStackTrace();
			return null;
	...
	public static ErrorReport withException(String cause, Throwable error) {
		...
		configuration = configuration.replaceAll("password: (.*)", "password: XXXXXXXXXX"); // Attempt
																																												// to remove
																																												// any
																																												// passwords
																																												// from the
