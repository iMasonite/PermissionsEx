...
public abstract class PermissionBackend {
	...
	/*
	 * public List<PermissionsUserData> getUsers() { List<PermissionsUserData> userData = new
	 * ArrayList<PermissionsUserData>(); for (String name : getUserNames()) {
	 * userData.add(getUserData(name)); } return Collections.unmodifiableList(userData); }
	 */
	
	public abstract Collection<String> getGroupNames();
	
	/*
	 * public List<PermissionsGroupData> getGroups() { List<PermissionsGroupData> groupData = new
	 * ArrayList<PermissionsGroupData>(); for (String name : getGroupNames()) {
	 * groupData.add(getGroupData(name)); } Collections.sort(groupData); return
	 * Collections.unmodifiableList(groupData); }
	 */
	...
	/** Load data from alternate backend. Assume that this overwrites all data in the receiving backend
	 * (except for users not included in transferring backend)
	 * 
	...
	public static String getBackendClassName(String alias) {
		...
		if (REGISTERED_ALIASES.containsKey(alias)) return REGISTERED_ALIASES.get(alias).getName();
		
		return alias;
	}
	
	/** Returns Class object for specified alias, if there is no alias registered then try to find it
	 * using Class.forName(alias)
	 * 
	 * @param alias
	 * @return
	...
	public static Class<? extends PermissionBackend> getBackendClass(String alias) throws ClassNotFoundException {
		if (!REGISTERED_ALIASES.containsKey(alias)) {
			...
			if (!PermissionBackend.class.isAssignableFrom(clazz)) throw new IllegalArgumentException("Provided class " + alias + " is not a subclass of PermissionBackend!");
	...
	/** Return alias for specified backend class If there is no such class registered the fullname of
	 * this class would be returned using backendClass.getName();
	 * 
	 * @param backendClass
	...
	public static String getBackendAlias(Class<? extends PermissionBackend> backendClass) {
		if (REGISTERED_ALIASES.containsValue(backendClass)) {
			for (String alias : REGISTERED_ALIASES.keySet()) { // Is there better way to find key by
																													// value?
				if (REGISTERED_ALIASES.get(alias).equals(backendClass)) return alias;
			}
		}
	 ...
	 * @param manager PermissionManager object
	 * @param config Configuration object to access backend settings
	 * @param fallBackBackend name of backend that should be used if specified backend was not found
	...
	public static PermissionBackend getBackend(String backendName, PermissionManager manager, ConfigurationSection config, String fallBackBackend) throws PermissionBackendException {
		...
		try {
			...
			if (fallBackBackend == null) throw new RuntimeException(e);
			
			if (!className.equals(getBackendClassName(fallBackBackend))) return getBackend(fallBackBackend, manager, config, null);
			else throw new RuntimeException(e);
		}
		catch (Throwable e) {
			if (e instanceof InvocationTargetException) {
				e = e.getCause();
				if (e instanceof PermissionBackendException) throw ((PermissionBackendException) e);
			}
