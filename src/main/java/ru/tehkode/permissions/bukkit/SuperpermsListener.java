...
public class SuperpermsListener implements Listener {
	...
	public void onPlayerLogin(PlayerLoginEvent event) {
		try {
			...
			// Because player world is inaccurate in the login event (at least with MV), start with null
			// world and then reset to the real world in join event
	...
	@EventHandler(priority = EventPriority.MONITOR)
	// Technically not supposed to use MONITOR for this, but we don't want to remove before other
	// plugins are done checking permissions
	...
	public void onSystemEvent(PermissionSystemEvent event) {
		try {
			if (event.getAction() == PermissionSystemEvent.Action.DEBUGMODE_TOGGLE) return;
			switch (event.getAction()) {
