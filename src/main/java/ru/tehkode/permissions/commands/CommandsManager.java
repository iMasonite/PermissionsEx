...
public class CommandsManager {
	...
	public boolean execute(CommandSender sender, org.bukkit.command.Command command, String[] args) {
		...
		for (Entry<CommandSyntax, CommandBinding> entry : callMap.entrySet()) {
			...
			if (selectedBinding != null && syntax.getRegexp().length() < argumentsLength) { // match, but
																																											// there
																																											// already
																																											// more fitted
																																											// variant
		...
		if (sender instanceof Player) { // this method are not public and required permission
			if (!selectedBinding.checkPermissions((Player) sender)) {
				plugin.getLogger().warning("User " + sender.getName() + " tried to access chat command \"" + command.getName() + " " + arguments + "\", but doesn't have permission to do this.");
			...
			if (this.arguments.size() > 0) {
				...
				if (argMatcher.find()) {
					for (int index = 1; index <= argMatcher.groupCount(); index++) {
						...
						if (argumentValue.startsWith("\"") && argumentValue.endsWith("\"")) { // Trim boundary
																																									// colons
