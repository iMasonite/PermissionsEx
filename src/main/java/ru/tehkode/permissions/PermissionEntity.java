...
public abstract class PermissionEntity {
	...
	/** This method 100% run after all constructors have been run and entity object, and entity object
	 * are completely ready to operate */
	public void initialize() {
		this.debugMode = this.getOptionBoolean("debug", null, this.debugMode);
	}
	
	/** Return name of permission entity (User or Group) User should be equal to Player's name on the
	 * server
	 * 
	 * @return name */
	...
	/** Return non-inherited user prefix. This means if at entity don't have has own prefix then empty
	 * string or null would be returned
	 * 
	...
	/** Return non-inherited user suffix. This means if an entity don't has own suffix then empty
	 * string or null would be returned
	 * 
	...
	public boolean getOptionBoolean(String optionName, String world, boolean defaultValue) {
		...
		if ("false".equalsIgnoreCase(option)) return false;
		else if ("true".equalsIgnoreCase(option)) return true;
	...
	public List<String> getTimedPermissions(String world) {
		...
		if (!this.timedPermissions.containsKey(world)) return Collections.emptyList();
	...
	public int getTimedPermissionLifetime(String permission, String world) {
		...
		if (!this.timedPermissionsTime.containsKey(world + ":" + permission)) return 0;
	 ...
	 * @param permission
	 * @param world
	 * @param lifeTime Lifetime of permission in seconds. 0 for transient permission (world disappear
	 *        only after server reload) */
	...
	public void removeTimedPermission(String permission, String world) {
		...
		if (!this.timedPermissions.containsKey(world)) return;
	...
	public String getMatchingExpression(List<String> permissions, String permission) {
		for (String expression : permissions) {
			if (isMatches(expression, permission, true)) return expression;
		}
	 ...
	 * 
	 * @param expression permission expression - what user have in his permissions list
	 *        (permission.nodes.*)
	...
	public boolean explainExpression(String expression) {
		...
		return !expression.startsWith("-"); // If expression have - (minus) before then that mean
	...
	protected List<PermissionGroup> getParentsInternal(String world) {
		...
		new HierarchyTraverser<Void>(this, world, false) { // Must not traverse inheritance or bad
																												// things happen :)
			@Override
			protected Void fetchLocal(PermissionEntity entity, String world) {
