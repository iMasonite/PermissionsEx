...
public class PermissionManager {
	...
	public PermissionUser getUser(UUID uid) {
		...
		OfflinePlayer ply = plugin.getServer().getPlayer(uid); // to make things cheaper, we're just
																														// checking online players (can be
																														// improved later on)
	...
	public PermissionGroup getGroup(String groupname) {
		...
		if (group == null) {
			...
			else throw new IllegalStateException("Group " + groupname + " is null");
		}
	 ...
	 * 
	 * @param ladderName
	 * @return Map of ladder, key - rank of group, value - group object. Empty map if ladder does not
	 *         exist */
	...
	protected void registerTask(TimerTask task, int delay) {
		if (timer == null || delay == TRANSIENT_PERMISSION) return;
