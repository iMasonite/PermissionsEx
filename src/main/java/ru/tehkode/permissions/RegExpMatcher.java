...
public class RegExpMatcher implements PermissionMatcher {
	...
	public static String prepareRegexp(String expression) {
		...
		/*
		 * try { Matcher rangeMatcher = rangeExpression.matcher(regexp); while (rangeMatcher.find()) {
		 * StringBuilder range = new StringBuilder(); int from =
		 * Integer.parseInt(rangeMatcher.group(1)); int to = Integer.parseInt(rangeMatcher.group(2)); if
		 * (from > to) { int temp = from; from = to; to = temp; } // swap them range.append("("); for
		 * (int i = from; i <= to; i++) { range.append(i); if (i < to) { range.append("|"); } }
		 * range.append(")"); regexp = regexp.replace(rangeMatcher.group(0), range.toString()); } }
